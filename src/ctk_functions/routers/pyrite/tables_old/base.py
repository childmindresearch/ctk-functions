...
class BaseTable(Generic[T], abc.ABC):
    ...
    @property
    @abc.abstractmethod
    def _statement(self) -> sqlalchemy.Select[tuple[T]]:
    ...
    def data(self) -> T | None:
        ...
        with client.get_session() as session:
            self._data = session.execute(self._statement).scalar_one_or_none()
        self._has_fetched_data = True

        return self._data

    @property
    def data_no_none(self) -> T:
...
class TScoreTable(BaseTable[T], abc.ABC):
        ...
        for index, label in enumerate(self._row_labels):
            ...
            for relevance in label.relevance:
                if relevance.in_range(score):
                    extended_cell = cmi_docx.ExtendCell(
                        table.template_rows[index + 1].cells[1],
                    )
...
class ParentChildTable(BaseTable[T], abc.ABC):
    ...
    def _add(
        ...
        for index, label in enumerate(self._row_labels):
            row = table.template_rows[index + 1].cells
